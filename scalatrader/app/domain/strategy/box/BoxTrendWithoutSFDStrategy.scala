...
class BoxTrendWithoutSFDStrategy(st: StrategyState, user: User) extends Strategy(st, user) {
  ...
  override def judgeByTicker(ticker: Ticker): Option[Ordering] = {
    ...
    val result = if (!isAvailable || Strategies.coreData.box1h.isEmpty || momentum.isEmpty) {
      ...
      if (state.order.isEmpty) {
        if (!sfd.aroundSFD && box10min.isUp && box20min.isUp && box1h.isUp && isUpdatingHigh) {
          entry(Buy)
        } else if (!sfd.aroundSFD && box10min.isDown && box20min.isDown && box1h.isDown && isUpdatingLow) {
          entry(Sell)
        } else {
          None
        }
